### Note: Some flags are missing in this list due to some recent fixes to the bot's file compiler. Those flags will be added back the next time the bot is run.
...
| `DockEnabled_ServerControlDock` | `false` |  |  | [Removed] | Toggles whether or not the corresponding dock will be loaded. This is used as a hard on-off switch for the corresponding dock. |
...
| `Game_ForcedTemporarilySavedDataExpiration` | `86100` |  |  | `3600` | Controls how long until data that is usually saved without an expiration date gets automatically removed when it is forced to be temporary. |
